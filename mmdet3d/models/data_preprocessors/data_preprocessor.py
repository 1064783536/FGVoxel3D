...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def __init__(self,
                 voxel: bool = False,
                 voxel_type: str = 'hard',
                 voxel_layer1: OptConfigType = None,
                 voxel_layer4: OptConfigType = None,
                 batch_first: bool = True,
                 max_voxels: Optional[int] = None,
                 mean: Sequence[Number] = None,
                 std: Sequence[Number] = None,
        ...
        self.max_voxels = max_voxels
        if voxel:
            self.voxel_layer1 = VoxelizationByGridShape(**voxel_layer1)
            self.voxel_layer4 = VoxelizationByGridShape(**voxel_layer4)
    ...
    def voxelize(self, points: List[Tensor],
        ...
        #     voxels, coors, num_points, voxel_centers = [], [], [], []
        #     for i, res in enumerate(points):
        #         res_voxels, res_coors, res_num_points = self.voxel_layer1(res)
        #         res_voxel_centers = (
        #             res_coors[:, [2, 1, 0]] + 0.5) * res_voxels.new_tensor(
        #                 self.voxel_layer1.voxel_size) + res_voxels.new_tensor(
        #                     self.voxel_layer1.point_cloud_range[0:3])
        #         res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
        #         voxels.append(res_voxels)
        #         coors.append(res_coors)
        #         num_points.append(res_num_points)
        ...
        if self.voxel_type == 'hard':
            ...
            voxel_dict['num_points1'] = num_points
            voxel_dict['voxel_centers1'] = voxel_centers
            voxel_dict['voxels1'] = voxels
            voxel_dict['coors1'] = coors

            #  branch 4
            voxels, coors, num_points, voxel_centers = [], [], [], []
            for i, res in enumerate(points):
                res_voxels, res_coors, res_num_points = self.voxel_layer4(res)
                res_voxel_centers = (
                    res_coors[:, [2, 1, 0]] + 0.5) * res_voxels.new_tensor(
                        self.voxel_layer4.voxel_size) + res_voxels.new_tensor(
                            self.voxel_layer4.point_cloud_range[0:3])
                res_coors = F.pad(res_coors, (1, 0), mode='constant', value=i)
                voxels.append(res_voxels)
                coors.append(res_coors)
                num_points.append(res_num_points)
            ...
            voxel_dict['num_points4'] = num_points
            voxel_dict['voxel_centers4'] = voxel_centers
            voxel_dict['voxels4'] = voxels
            voxel_dict['coors4'] = coors
